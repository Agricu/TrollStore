...
- (NSError*)loadInfoDictionary
{
	...
	{
		NSString* mainInfoPlistPath = [NSString stringWithFormat:NSLocalizedString(@"Payload/%@/Info.plist",nil), _cachedAppBundleName];
	...
	else
	{
		...
		{
			NSString* pluginSubpath = [NSString stringWithFormat:NSLocalizedString(@"PlugIns/%@",nil), plugIn];
	...
	if(!_cachedEntitlementsByBinarySubpaths)
	{
		...
		if(_isArchive)
		{
			if(_cachedInfoDictionary)
			{
				...
				NSString* bundleExecutableSubpath = [NSString stringWithFormat:NSLocalizedString(@"Payload/%@/%@",nil), _cachedAppBundleName, bundleExecutable];
...
- (void)enumerateAvailableIcons:(void (^)(CGSize iconSize, NSUInteger iconScale, NSString* iconPath, BOOL* stop))enumerateBlock
{
	...
	if(_cachedInfoDictionary)
	{
		...
		if(_isArchive)
		{
			NSString* iconPrefix = [NSString stringWithFormat:NSLocalizedString(@"Payload/%@/%@",nil), _cachedAppBundleName, iconName];
			[self enumerateArchive:^(struct archive_entry* entry, BOOL* stop)
			{
				NSString* currentSubpath = [NSString stringWithUTF8String:archive_entry_pathname(entry)];
...
- (NSAttributedString*)detailedInfoDescription
{
	...
	if(unrestrictedContainerAccess || accessibleContainers.count)
	{
		...
		else
		{
			...
			{
				[description appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:NSLocalizedString(@"\n%@",nil), containerID] attributes:bodyAttributes]];
			}
		}
	}
